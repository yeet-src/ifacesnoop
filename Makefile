...
all: ifacesniff.bpf.o

# struct __sk_buff comes from the running kernel's BTF — CO-RE, no system
# linux/*.h headers needed.
include/vmlinux.h:
	@mkdir -p include
	$(BPFTOOL) btf dump file /sys/kernel/btf/vmlinux format c > $@

ifacesniff.bpf.o: ifacesniff.bpf.c include/vmlinux.h
	$(CLANG) $(CFLAGS) -c $< -o $@

clean:
	rm -f ifacesniff.bpf.o include/vmlinux.h
